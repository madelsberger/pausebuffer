  ...
  return function (user, message) {
    if (canWhisperTo(user)) {
      ...
      if (!nextWhisperTime) {
        ...
        return client.whisper(user, message);
      } else {
        const delay = Math.max(0, nextWhisperTime - Date.now());
        nextWhisperTime += 750;
        return new Promise(r => setTimeout(() =>
                                     r(client.whisper(user, message)), delay));
      }
    }
  };
}

const canWhisperTo = function (username, reserve = false) {
  if (whisperRecipients.size > 39 && !whisperRecipients.has(username)) {
    return false
  }
  if (reserve) {
    whisperRecipients.add(username);
  }
  return true;
}
